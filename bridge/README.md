...
4. **行为面最小**：无桌面操控、不自启动、无静默后台；**默认不请求管理员权限，双击（交互模式）自动提权**（v0.6.3+，UAC 用户确认）

## 管理员提权（v0.6.3+）

读取完整 BIOS 设置（`Lenovo_BiosSetting`）等操作需要管理员权限。bridge **双击默认自动提权**（方案 A）：

```bash
# 双击运行 → 自动弹 UAC → 点"是" → 管理员运行（无需任何额外操作）
# 点"否" → 降级普通权限继续运行

# 命令行模式：默认不提权，显式加 --elevate 才提权
bridge -server wss://your-server:8000 -room 房间码 --elevate

# 特殊情况禁止提权
bridge -server wss://your-server:8000 -room 房间码 --no-elevate
```
