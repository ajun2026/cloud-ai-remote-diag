## v0.10.0 — 2026-08-10（用户反馈模块）

### 新增功能

工程师可在工作台提交测试反馈，管理员在管理后台统一查看：

- **工作台「📝 反馈」页**：左侧导航新增反馈入口，填写内容提交，可查看自己的历史反馈记录（三语支持）
- **后端**：新增 `feedbacks` 表 + 3 个 API（提交 `POST /api/feedback`、我的记录 `GET /api/feedback/my`、管理员查看 `GET /api/admin/feedbacks`），提交需登录、内容限 2000 字
- **管理后台**：新增「用户反馈」区块，展示全部反馈（ID / 工号 / 内容 / 时间）

### 版本号

v0.9.4 → v0.10.0（server.py 5 处 + dashboard.html 1 处）
